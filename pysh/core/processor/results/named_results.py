...
class NamedResults(results.Results[result.Result],Mapping[str,result.Result]):
    _results: Mapping[str,result.Result]
    ...
    def single(self)->'single_result.SingleResult[result.Result]':
        if len(self) != 1:
            raise error.Error(result=self, msg=f'unable to convert NamedResult to SingleResult: invalid len {len(self)}')
        else:
            return single_result.SingleResult[result.Result](list(self._results.values())[0])
        
    def optional(self)->'optional_result.OptionalResult[result.Result]':
        if len(self) > 1:
            raise error.Error(result=self, msg=f'unable to convert NamedResult to SingleResult: invalid len {len(self)}')
        elif len(self) == 1:
            return optional_result.OptionalResult[result.Result](list(self._results.values())[0])
        else:
            return optional_result.OptionalResult[result.Result]()
        
    def multiple(self)->'multiple_results.MultipleResults[result.Result]':
        return multiple_results.MultipleResults[result.Result](list(self._results.values()))
    
    def named(self, name: str)->'NamedResults[result.Result]':
        return self
    

from pysh.core.processor.results import no_result, single_result, optional_result, multiple_results